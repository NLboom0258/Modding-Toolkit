...
STRINGS = {
    ...
    # ── ui/name_cleanup.py — 非 ASCII 骨骼/顶点组名清理 ─────────────────────
    "ui.main_panel.name_cleanup_header": {"EN": "Non-ASCII Name Cleanup", "ZH": "非ASCII命名清理"},
    "ui.name_cleanup.no_armature": {"EN": "Select an armature to scan",
                                    "ZH": "请选中一个骨架以扫描"},
    "ui.name_cleanup.all_ascii": {"EN": "No non-ASCII bone or vertex group names",
                                  "ZH": "未发现非 ASCII 骨骼/顶点组名"},
    "ui.name_cleanup.found": {"EN": "{count} non-ASCII name(s) found",
                              "ZH": "发现 {count} 个非 ASCII 命名"},
    "ui.name_cleanup.none_after_filter": {"EN": "No name matches the filter",
                                         "ZH": "没有名字匹配该搜索"},
    "mtk.name_cleanup.err_empty": {"EN": "The name cannot be empty",
                                   "ZH": "名字不能为空"},
    "mtk.name_cleanup.err_ascii": {"EN": "The name must be ASCII",
                                   "ZH": "名字必须是 ASCII"},
    "mtk.name_cleanup.err_bone_conflict": {"EN": "A bone named '{name}' already exists",
                                           "ZH": "已存在同名骨骼 '{name}'"},
    "mtk.name_cleanup.err_group_conflict": {"EN": "A vertex group named '{name}' already exists",
                                            "ZH": "已存在同名顶点组 '{name}'"},
    "mtk.name_cleanup.renamed": {"EN": "Renamed '{old}' to '{new}'",
                                 "ZH": "已把 '{old}' 改名为 '{new}'"},
    "mtk.name_cleanup.selected": {"EN": "Selected bone '{name}'",
                                  "ZH": "已选中骨骼 '{name}'"},
    "mtk.name_cleanup.deleted": {"EN": "Deleted '{name}'; {count} child bone(s) re-parented to '{parent}'",
                                  "ZH": "已删除 '{name}'；{count} 个子骨骼已接到 '{parent}'"},
    "mtk.name_cleanup.deleted_root": {"EN": "Deleted '{name}' (no parent); {count} child bone(s) became roots",
                                       "ZH": "已删除 '{name}'（无父骨骼）；{count} 个子骨骼变为根骨骼"},
    "mtk.name_cleanup.merged": {"EN": "Merged '{name}' into '{parent}'",
                                 "ZH": "已把 '{name}' 合并进 '{parent}'"},
    "mtk.name_cleanup.err_no_bone": {"EN": "No bone named '{name}'",
                                      "ZH": "没有名为 '{name}' 的骨骼"},
    "mtk.name_cleanup.err_no_parent": {"EN": "'{name}' has no parent bone, so it cannot be merged -- delete or rename it instead",
                                        "ZH": "'{name}' 没有父骨骼，无法合并 —— 请改用删除或改名"},
    "mtk.name_cleanup.confirm_delete_title": {"EN": "Delete bone", "ZH": "删除骨骼"},
    "mtk.name_cleanup.confirm_delete_msg": {"EN": "Delete '{name}' and its weights? Its child bones are re-parented to its parent.",
                                             "ZH": "删除 '{name}' 及其权重？它的子骨骼会被接到它的父级。"},
    "mtk.name_cleanup.confirm_merge_title": {"EN": "Merge bone", "ZH": "合并骨骼"},
    "mtk.name_cleanup.confirm_merge_msg": {"EN": "Merge '{name}' into '{parent}'? Its weights are added to the parent's group.",
                                            "ZH": "把 '{name}' 合并进 '{parent}'？它的权重会加到父骨骼的顶点组上。"},
